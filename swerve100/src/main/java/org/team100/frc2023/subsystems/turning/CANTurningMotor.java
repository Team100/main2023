...
public class CANTurningMotor implements TurningMotor {
    ...
    m_motor.configAllowableClosedloopError(0, 0, 30);
        ...
        double absolutePosition = m_encoder.get()*1666;
    ...
    public void setPID(ControlMode control, double output) {
        ...
        double goal =  m_motor.getSelectedSensorPosition() + MathUtil.angleModulus(2*Math.PI*(output - m_motor.getSelectedSensorPosition()/rotationsPerRev))/(2*Math.PI)*rotationsPerRev;
