 ...
 */
public class SwerveSetpointGenerator2 {
    ...
    public SwerveSetpointGenerator2(final SwerveDriveKinematics kinematics) {
    ...
    public SwerveSetpoint generateSetpoint(final KinematicLimits limits, final SwerveSetpoint prevSetpoint, ChassisSpeeds desiredState, double dt) {
        ...
        for (int i = 0; i < modules.length; ++i) {
            ...
            double s = findDriveMaxS(prev_vx[i], prev_vy[i], Math.hypot(prev_vx[i], prev_vy[i]),
